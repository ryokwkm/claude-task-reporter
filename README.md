...
Claude Code がタスクを完了したタイミングで、セッションのトランスクリプトを `claude-haiku` または `gemini-cli` で要約し、macOS のデスクトップ通知と音声読み上げで報告する Stop フック。
